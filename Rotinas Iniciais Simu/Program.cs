...
namespace Rotinas_Iniciais_Simu
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            //===================== LOOP DO Q LEARNING NORMAL COM INFO INCOMPLETA - 2 Diamantes =======================
            env_incomplete_two_diamonds env2 = new env_incomplete_two_diamonds();
            Console.WriteLine("Treinando o agente por meio do Q-Learning no ambiente com informacao totalmente incompleta -> Com apenas 2 diamantes");
            float[,] qTableIncomplete2 = makeQtableIncomplete();
            int max_steps;
            for (int ep = 0; ep < episodes; ep++)
            {
                ...
                (state, done) = env2.reset();
                max_steps = 0;
                while (!done && max_steps < 12)
                {
                    action = get_actionQIncomplete(state, rnd, qTableIncomplete2);
                    max_steps++;

                    (next_state, reward, done) = env2.step(state, action);
                    if (next_state[0,0] == 16)
                    {
                        
                    }

                    num = learning_rate_Q * (reward + gamma* MaxTableIncomplete(next_state, qTableIncomplete2) - qTableIncomplete2[(int)state[0, 0]-1, action]);
                    qTableIncomplete2[(int)state[0, 0]-1, action]
                        = num +  qTableIncomplete2[(int)state[0, 0]-1, action];
            ...
            {
                max_steps++;
                action = get_actionQIncomplete(state, rnd, qTableIncomplete2, false);
                string direcao = "nada";
                if (action == 0)
                {
                    direcao = "esquerda";
